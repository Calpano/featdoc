 ...
 * @param targetSystem consumed the message. Can be null for outgoing messages.
...
public record ResultStep(ScenarioStep scenarioStep, int depth, System sourceSystem, Message message,
                         String messageComment, @Nullable System targetSystem, @Nullable Rule rule) {
    ...
    public ResultStep {
        if(targetSystem==null && !(message.isOutgoing() && message.system().equals(sourceSystem))) {
            throw new IllegalArgumentException("Target system is null, but message is '"+message+"' from source system '"+sourceSystem+"'");
        }
    }
    ...
    public String toString() {
        return String.format("%-10s --> %-10s : Msg=%-40s | Feat=%-20s | depth=%s | %s",
                sourceSystem().label,
                targetSystem==null? "--": targetSystem.label,
